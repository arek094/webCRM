...
switch ($source_data) {
    ...
    case 'wydanie_poz_dost_zam_poz':
        ...
        $output = GetDataQuery('SELECT  k_zam_poz_raport.numer_zam,
                                ...
                                LEFT JOIN (
                                            SELECT wytwor_id,  zam_poz_dost_id, SUM(ilosc_potwierdzona) as ilosc_potwierdzona, SUM(ilosc) as ilosc_wyd_zatw FROM t_wydanie_poz_dost 
                                            ...
                                            GROUP BY wytwor_id, zam_poz_dost_id) wyd_zatw
                                ON  k_zam_poz_raport.zam_poz_dost_id =  wyd_zatw.zam_poz_dost_id  
                                LEFT JOIN (
                                            SELECT wytwor_id, zam_poz_dost_id, SUM(ilosc_potwierdzona) as ilosc_wyd_pzyj, SUM(ilosc) as ilosc FROM t_wydanie_poz_dost 
                                            INNER JOIN t_wydanie_nag_dost ON t_wydanie_poz_dost.wydanie_nag_dost_id = t_wydanie_nag_dost.wydanie_nag_dost_id
                                            WHERE t_wydanie_nag_dost.status_id = 2 OR t_wydanie_nag_dost.status_id = 3
                                            GROUP BY wytwor_id, zam_poz_dost_id) wyd_pzyj
    ...
    case 'k_zam_poz_raport':
        ...
        break;    
    case 'k_zam_eksport_csv':
        $output = GetDataQuery('SELECT wytwor_idm,wytwor_nazwa,ilosc_oczekiwana,cena_netto FROM k_zam_eksport_csv WHERE zam_nag_dost_id='.$id.'',$uzytkownik_id);
        break;  
    case 'k_zam_poz_etykiety_bez_powtarzania':
        $output = GetDataQuery('SELECT DISTINCT kontrahent_id,kontrahent_nazwa,zam_nag_dost_id,wytwor_id,wytwor_idm,wytwor_nazwa,kod_wytwor FROM k_zam_poz_etykiety WHERE zam_nag_dost_id='.$id.'',$uzytkownik_id);
        break;       
    case 'k_zam_poz_etykiety':
        $output = GetDataQuery('SELECT * FROM k_zam_poz_etykiety WHERE zam_nag_dost_id='.$id.'',$uzytkownik_id);
        break; 
    default:
