...
switch ($source_data) {
    case 'k_wydanie_nag_raport_k_wydanie_poz_raport':
        $output = GetDataQueryPosition('SELECT * FROM k_wydanie_nag_raport WHERE wydanie_nag_dost_id='.$id.'','SELECT *,@lp := @lp + 1 AS lp  FROM k_wydanie_poz_raport, (SELECT @lp := 0) temp WHERE wydanie_nag_dost_id=','wydanie_nag_dost_id',$uzytkownik_id);
        break;  
    ...
    case 'k_wydanie_poz_dost_szczegoly':
        ...
        break;
    case 'k_wydanie_poz_dost_temp_szczegoly':
