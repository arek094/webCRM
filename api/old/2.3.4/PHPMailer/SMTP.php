...
class SMTP
{
     ...
     * @see    hello()
     *
     * @param string $nazwa_uzytkownika The user name
     * @param string $haslo The haslo
    ...
    public function authenticate(
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                // Send encoded nazwa_uzytkownika and haslo
                if (!$this->sendCommand(
                    'User & Password',
                    base64_encode("\0" . $nazwa_uzytkownika . "\0" . $haslo),
                    235
                )
                ) {
                    return false;
            ...
            case 'LOGIN':
                ...
                if (!$this->sendCommand('Username', base64_encode($nazwa_uzytkownika), 334)) {
                    return false;
                }
                if (!$this->sendCommand('Password', base64_encode($haslo), 235)) {
                    return false;
                }
                break;
            case 'CRAM-MD5':
                ...
                // Build the response
                $response = $nazwa_uzytkownika . ' ' . $this->hmac($challenge, $haslo);
