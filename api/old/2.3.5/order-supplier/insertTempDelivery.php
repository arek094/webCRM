...
if(isset($uzytkownik_id)){
    ...
    $ostatni_numer = mysqli_query($mysqli,"SELECT SUBSTRING(numer, LOCATE('/',numer,5)+1, 4) as ostatni_numer FROM t_wydanie_nag_dost WHERE SUBSTRING(numer, LOCATE('/',numer) + 5, 4) = YEAR(NOW()) order by SUBSTRING(numer, LOCATE('/',numer,5) + 1, 4) DESC LIMIT 1");
    ...
    $kontrahent_id = mysqli_query($mysqli,"SELECT kontrahent_id FROM t_uzytkownicy WHERE uzytkownik_id='$uzytkownik_id'");
    $kontrahent_id_wiersz = mysqli_fetch_array($kontrahent_id,MYSQLI_ASSOC);
    $kontrahent_id_wiersz_check = isset($kontrahent_id_wiersz["kontrahent_id"]) ? $kontrahent_id_wiersz["kontrahent_id"] : 0;
    ...
    $miesiac = date("m");
    $rok = date("Y");
    $numer = 'WZ1'.'/'.$dzien.$miesiac.$rok.'/'.$numer.'/'.$kontrahent_id_wiersz_check;
