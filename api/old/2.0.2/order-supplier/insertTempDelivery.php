...
//$tempDelivery = [...array_filter($tempDelivery, "spr_ilosc_wydana")];
$tempDelivery = array_merge(array_filter($tempDelivery, "spr_ilosc_wydana"));

if(isset($uzytkownik_id)){
    

    $ostatni_numer = mysqli_query($mysqli,"SELECT SUBSTRING(numer, LOCATE('/',numer,4) + 10, 5) as ostatni_numer FROM t_wydanie_nag_dost WHERE SUBSTRING(numer, LOCATE('/',numer,2) + 5, 4) = YEAR(NOW()) order by numer DESC LIMIT 1");
