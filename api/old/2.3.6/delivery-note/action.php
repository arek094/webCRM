...
if (isset($uzytkownik_id)){
    ...
    switch ($action) {
        case 1:
            ...
            break;
        case 2:
            $query_insert_nag="INSERT INTO t_wydanie_nag_dost (uzytkownik_id_utworzenia, numer,numer_wz_obcy)
                    SELECT uzytkownik_id_utworzenia, numer, numer_wz_obcy
            ...
            $query_insert_poz="INSERT INTO t_wydanie_poz_dost (wydanie_nag_dost_id, nr_poz , wytwor_id,ilosc,zam_poz_dost_id)
                                ...
                                FROM (
                                        ...
                                        ORDER BY zam_poz_dost_id, uwagi_poz asc
                                ) as wyd";
        ...
        case 3:
            ...
            break;    
        case 4:
            $query="UPDATE t_wydanie_nag_dost SET status_id=2 , uzytkownik_id_przyjecia= '$uzytkownik_id', data_przyjecia = NOW() WHERE wydanie_nag_dost_id='$deliverydata'";
            $result=mysqli_query($mysqli,$query);
            ...
            break;  
        case 5:
            $numer_obcy = $deliverydata["numer_obcy"] ?? NULL;
            $wydanie_nag_id = $deliverydata["wydanie_nag_id"] ?? NULL;
            
            $query="UPDATE t_wydanie_nag_dost_temp SET numer_wz_obcy='$numer_obcy' WHERE wydanie_nag_dost_id='$wydanie_nag_id'";
            $result=mysqli_query($mysqli,$query);
        
            if (isset($result)){            
                $output = array('error'  => false,'message' => 'Wydanie zostało poprawnie zapisane.');
            } else{
                $output = array('error'  => true,'message' => 'Błąd podczas zatwierdzania.');
            }
